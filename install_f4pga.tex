...
While most FPGA architectures are not open source, F4PGA has functional flows for both the more open companies like Lattice 
and more more closed companies like Xilinx.
Project x-ray is an attempt to reverse engineer the Xilinx architectures from compiled bitstreams to create functional open source toolchains.
Unfortunately at time of writing F4PGA is not pre-compiled for any ARM based cpu platforms; 
however, this is luckily not a deal-breaker for us.
...
So instead to get this working you are going to need to use Apple's Rosetta for x86 emulation, 
...
while this works, it is less than ideal due to the high overhead to run Windows within macOS, 
